...
<body>
    ...
    <div id="main-wrapper">
        ...
        <div class="page-wrapper">
             ...
             <div class="row">
                    <div class="col-12">
                        <div class="card border border-info m-5 bg-default p-4">
                        	<div class="row">
                        		<div class="col-sm-10 col-10"><h2 class="h2 text-bold text-info">Booking List</h2></div>
                            ...
                            <div class="table-responsive">
                             ...
                             <table id="zero_config" class="table table-striped table-bordered display nowrap table-sm p-3" style="width: 100%;">
            ...
            if($status=="customer" && $user_type=="Individual"){
                  $sql="SELECT * FROM `booking` INNER JOIN `users` ON `booking`.`contact_name` = `users`.`username` WHERE `booking`.`contact_name`='$user'";
                  ?>
                   <thead>
                                            <tr>
                                                <th class="text-nowrap">#</th>
                                                <th class="text-nowrap">Conatct Name</th>
                                                <th>Address</th>
                                                <th>Contact</th>
            ...
            while($rows =mysqli_fetch_array($run_sql)){
                ...
                echo '
                        <tr>
                            ...
                            <td class="text-nowrap">'.$rows['address'].'</td>
                            <td class="text-nowrap">'.$rows['contact'].'</td>
                            
                            <td class="text-nowrap">'.$rows['service'].'</td> 
                             <td class="text-nowrap">'.$rows['date'].'</td> <td  
                             <td class="text-nowrap">'.$rows['next_date'].'</td> <td class="text-nowrap">'.$rows['time'].'</td> <td class="text-nowrap"><span class="badge badge-success">Booked</span></td> 
                           '; 
                            
                         
                            echo '<td><a class="btn btn-danger text-white btn-sm" onclick="book_del('.$rows['id'].')"><i class="mdi mdi-delete"></i></a>&nbsp&nbsp &nbsp&nbsp<a class="btn btn-dark text-white btn-sm" href=""><i class="mdi mdi-account-edit"></i></a>
            ...
            elseif($status=="customer" && $user_type=="Company"){
                  ?>
                     <thead>
                                            <tr>
                                                <th class="text-nowrap">#</th>
            ...
            while($rows =mysqli_fetch_array($run_sql)){
                ...
                echo '
                        <tr>
                            ...
                            <td class="text-nowrap">'.$rows['address'].'</td>
                            <td class="text-nowrap">'.$rows['contact'].'</td>
                            <td class="text-nowrap">'.$rows['in_charge'].'</td>
                            <td class="text-nowrap">'.$rows['manager'].'</td>
                            <td class="text-nowrap">'.$rows['garage_name'].'</td>
                            <td class="text-nowrap">'.$rows['no_of_vehicles'].'</td>
                            
                            <td class="text-nowrap">'.$rows['service'].'</td> 
                             <td class="text-nowrap">'.$rows['date'].'</td> <td  
                             <td class="text-nowrap">'.$rows['next_date'].'</td> 
                             <td class="text-nowrap">'.$rows['time'].'</td>

                             <td class="text-nowrap"><span class="badge badge-success">Booked</span></td> 
                           '; 
              ...
              echo '<tfoot><tr>
                 ...
                 ?>
                      <thead>
                                        ...
                                        <tbody>
                 <?php
                 $sql="SELECT * FROM `booking` INNER JOIN `users` ON `booking`.`contact_name` = `users`.`username` ";
                   $run_sql=mysqli_query($conn,$sql);
            $i=0;
            while($rows =mysqli_fetch_array($run_sql)){
                $id=$rows['id'];
...
<script type="text/javascript">
      ...
      {
              toastr.success(data, 'A1AUTOGURU', { "showMethod": "fadeIn", "hideMethod": "fadeOut", timeOut: 2000 });
              location.reload();
          }
      });
